...
public class UserDaoImpl implements UserDao {
	...
	public User findUserById(Long id) {
		User user = this.jdbcTemplate.queryForObject("select user_id, name, age, email from user where user_id = ?",
	...
	public List<User> findUsers() {
		List<User> users = this.jdbcTemplate.query("select user_id, name, age, email from user", new UserMapper());
		return users;
	}
	...
	public Integer update(User user) {
		...
		return this.jdbcTemplate.update("update user set name = ?, age =?, email =? where user_id = ?", user.getName(),
				user.getAge(), user.getEmail(), user.getUserId());
	}
